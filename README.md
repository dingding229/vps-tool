...
VPS 到手后的一键初始化脚本。当前版本 `v0.7.1`，使用 Shell 编写，提供美化交互界面。
...
- 开机服务会等待内核模块和 sysctl 初始化，验证失败后支持延迟重试；当前运行状态正常时会自动修正旧的失败状态
...
6. 新系统启动后等待 systemd 模块和 sysctl 初始化，再检查是否进入 `joeyblog-bbrv3` 内核；
7. 写入 `/etc/sysctl.d/99-vps-tool-bbr.conf`，启用 `BBR + FQ`，运行状态尚未就绪时每 5 秒重试一次，最多 6 次；
8. 完成或失败后自动移除一次性恢复服务，并在下次运行 `sudo vps-tool` 时显示结果；
9. 如果开机检查曾过早失败，但当前内核、BBR v3、`bbr` 和 `fq` 已全部正常，状态页会自动把旧的失败记录修正为“验证通过”。
